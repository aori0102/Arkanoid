...
public class PlayerSkillsHandler extends MonoBehaviour {
     ...
     * - Registers a listener to the player's key press events.
     * - When a key is pressed, handleSkillRequest() will be triggered to invoke the corresponding skill.
     */
    @Override
    public void awake() {
    ...
    private void handleSkillRequest(Object o, ActionMap.Action action) {
        switch (action) {
            case Skill1 -> handleLogicBasedSkill(SkillIndex.LaserBeam);  // Q key
            case Skill2 -> handleLogicBasedSkill(SkillIndex.Updraft);    // E key
            case Skill3 -> handleLogicBasedSkill(SkillIndex.Invincible); // X key
            case Dash   -> handleLogicBasedSkill(SkillIndex.Dash);       // SHIFT key
        }
    }
