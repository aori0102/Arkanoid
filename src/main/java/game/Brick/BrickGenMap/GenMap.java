...
public final class GenMap extends InitBrick {
    ...
    public BrickMatrix generate(Style style, double difficulty) {
        ...
        return switch (style) {
            ...
            case STRIPES   -> generateStripes(
                    new BrickObj.typeBrick[]{
                            BrickObj.typeBrick.Normal, BrickObj.typeBrick.Rock,
                            BrickObj.typeBrick.Steel,  BrickObj.typeBrick.Diamond},
                    3, difficulty);
            case CHECKER   -> generateChecker(BrickObj.typeBrick.Normal, BrickObj.typeBrick.Rock, 1, 1, difficulty);
    ...
    public BrickMatrix generateGradientVertical(double difficulty) {
        ...
        for (int r = 0; r < ROWS; r++) {
            ...
            BrickObj.typeBrick ty = pickByBias(bias);
    ...
    public BrickMatrix generateStripes(BrickObj.typeBrick[] palette, int stripeH, double difficulty) {
        if (palette == null || palette.length == 0) palette = new BrickObj.typeBrick[]{BrickObj.typeBrick.Normal};
        if (stripeH <= 0) stripeH = 2;
        BrickMatrix g = emptyGrid();
        for (int r = 0; r < ROWS; r++) {
            BrickObj.typeBrick ty = palette[(r / stripeH) % palette.length];
            for (int c = 0; c < COLS; c++) g.set(r, c, new BrickObj(ty));
        }
        sprinkleSpecialsByDifficulty(g, difficulty);
        return g;
    ...
    public BrickMatrix generateChecker(BrickObj.typeBrick a, BrickObj.typeBrick b,
    ...
    public BrickMatrix generateSymmetricRandom(double density, double hardnessBias) {
        ...
        for (int r = 0; r < ROWS; r++)
            for (int c = 0; c < half; c++)
                if (rng.nextDouble() < density) {
                    BrickObj.typeBrick t = pickByBias(hardnessBias);
    ...
    public BrickMatrix generateRooms(double difficulty) {
        BrickMatrix g = emptyGrid();
        fillAll(g, BrickObj.typeBrick.Normal);
        ...
        for (int k = 0; k < rooms; k++) {
            ...
            BrickObj.typeBrick wall = pickFromTopHard(lerpIndexTop(0.5 + 0.5 * difficulty));
            BrickObj.typeBrick fill = pickByBias(0.25 + 0.5 * difficulty);
        ...
        for (int r = 0; r < ROWS; r++)
            for (int c = 0; c < COLS; c++) {
                ...
                if (f <= 0.0) {
                    ...
                    g.set(r, c, new BrickObj(BrickObj.typeBrick.Normal));
                }
            }
        for (int r = 0; r < ROWS; r++)
            for (int c = 0; c < COLS; c++) {
    ...
    public BrickMatrix generateDungeon(double difficulty) {
        ...
        for (int k = 0; k < rooms; k++) {
            ...
            int r0 = rng.nextInt(Math.max(1, ROWS - h));
            int c0 = rng.nextInt(Math.max(1, COLS - w));
            BrickObj.typeBrick wall = pickFromTopHard(lerpIndexTop(0.55 + 0.45 * difficulty));
            drawRect(g, r0, c0, h, w, wall, true);
    ...
    public BrickMatrix generateRings(double difficulty) {
        ...
        for (int d = 0; d < layers; d += 2) {
            ...
            BrickObj.typeBrick wall = pickByBias(bias);
            drawRect(g, r0, c0, h, w, wall, true);
    ...
    private void setSpiralCell(BrickMatrix g, int r, int c, BrickObj.typeBrick wall,
    ...
    public BrickMatrix generateSine(double difficulty) {
        ...
        BrickObj.typeBrick wall = pickFromTopHard(lerpIndexTop(0.55 + 0.35 * difficulty));
        for (int c = 0; c < COLS; c++) {
    ...
    public BrickMatrix generateCross(double difficulty) {
        BrickMatrix g = emptyGrid();
        fillAll(g, BrickObj.typeBrick.Normal);
        BrickObj.typeBrick hard = pickFromTopHard(lerpIndexTop(0.55 + 0.4 * difficulty));
    ...
    public BrickMatrix generateMaze(double difficulty) {
        BrickMatrix g = emptyGrid();
        // start with walls
        BrickObj.typeBrick wall = pickFromTopHard(lerpIndexTop(0.55 + 0.4 * difficulty));
        fillAll(g, wall);
    ...
    public BrickMatrix generateCaves(double difficulty) {
        ...
        for (int r = 0; r < ROWS; r++)
            for (int c = 0; c < COLS; c++)
                if (wall[r][c]) {
                    BrickObj.typeBrick t = pickFromTopHard(lerpIndexTop(0.5 + 0.45 * difficulty));
                    g.set(r, c, new BrickObj(t));
    ...
    public BrickMatrix generateDiagonalStripes(double difficulty) {
        ...
        int thick  = (int) Math.max(1, Math.round(lerp(1, 2, difficulty)));
        BrickObj.typeBrick wall = pickFromTopHard(lerpIndexTop(0.55 + 0.3 * difficulty));
        for (int r = 0; r < ROWS; r++)
            for (int c = 0; c < COLS; c++) {
    ...
    public BrickMatrix generateStar(double difficulty) {
        ...
        BrickObj.typeBrick wall = pickFromTopHard(lerpIndexTop(0.6 + 0.35 * difficulty));
    ...
    private void drawRect(BrickMatrix g, int r0, int c0, int h, int w, BrickObj.typeBrick t, boolean hollow) {
    ...
    private void setIfIn(BrickMatrix g, int r, int c, BrickObj.typeBrick t) { if (inBounds(r,c)) g.set(r, c, new BrickObj(t)); }
    ...
    // pickers & specials
    private BrickObj.typeBrick pickByBias(double bias) {
        BrickObj.typeBrick[] arr = BrickObj.typeBrick.values();
        int maxIx = (int) Math.round(clamp01(bias) * (arr.length - 1));
        int ix = rng.nextInt(maxIx + 1);
        return arr[ix];
    }
    private BrickObj.typeBrick pickFromTopHard(double topFrac) {
        BrickObj.typeBrick[] arr = BrickObj.typeBrick.values();
    ...
    private BrickObj.typeBrick pickFromWeak(double difficulty) {
        double bias = clamp01(0.15 + 0.25 * difficulty);
        BrickObj.typeBrick t = pickByBias(bias);
        if (t == BrickObj.typeBrick.Steel || t == BrickObj.typeBrick.Diamond) t = BrickObj.typeBrick.Normal;
        return t;
    }
    private void sprinkleSpecialsByDifficulty(BrickMatrix g, double difficulty) {
        ...
        for (int r = 0; r < ROWS; r++)
            for (int c = 0; c < COLS; c++) {
                ...
                if (x < bombP) g.set(r, c, new BrickObj(BrickObj.typeBrick.Bomb));
                else if (x < bombP + giftP) g.set(r, c, new BrickObj(BrickObj.typeBrick.Gift));
                else if (x < bombP + giftP + otherP) {
                    BrickObj.typeBrick[] pool = {
                            BrickObj.typeBrick.Rocket, BrickObj.typeBrick.Angel,
                            BrickObj.typeBrick.Reborn, BrickObj.typeBrick.Ball
                    };
